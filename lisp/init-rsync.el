...
(defcustom auto-rsync-command-option "-avz" "rsync command option" :group 'auto-rsync)
  ...
  (let* ((normalized-alist (mapcar (lambda (x) (cons (file-name-as-directory (expand-file-name (car x)))
    ...
    (when sync-pair
      (save-window-excursion
        ...
        (shell-command-to-string (format "%s %s %s %s&" auto-rsync-command auto-rsync-command-option (car sync-pair) (cdr sync-pair)))))))
